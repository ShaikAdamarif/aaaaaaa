...
function adminDash(){
  ...
  app.innerHTML = dashHeader(s,'Admin Dashboard') + `
    ...
    <div class="tabs" style="margin-top:20px">
      ${['overview','manage','approvals','users','assign','submissions','offers','callbacks','attendance','company','portfolio','passwords']
...
function renderAdminTab(t){
  ...
  if(t==='overview'){
    ...
    const records=db.get(KEYS.attendance,[]);
    const logs=db.get(KEYS.sessionsLog,[]);
    ...
    c.innerHTML=`
      ...
      <div class="card" style="margin-top:14px">
        <h3>Export Attendance for a Person</h3>
        <div class="input-group"><label>Select Person (HR / User)</label>
          <select id="att_person">${people.map(u=>`<option value="${u.id}">${esc(u.name)} (${u.role.toUpperCase()})</option>`).join('')}</select>
        </div>
        <div class="btn-row">
          <button class="btn" onclick="window._av.exportAttendance('excel')">⬇ Export Excel</button>
          <button class="btn ghost" onclick="window._av.exportAttendance('pdf')">⬇ Export PDF</button>
        </div>
      </div>
      <div class="card" style="margin-top:14px">
        <h3>🔍 Search</h3>
        <div class="input-group"><label>Search attendance records (by name / email / role / date)</label>
          <input id="att_search" placeholder="e.g. Ravi, hr, 2026-05-16" oninput="window._av.filterAttList()"/></div>
        <div class="input-group"><label>Search login / logout timings (by name / email / role / date)</label>
          <input id="log_search" placeholder="e.g. Priya, user, 2026-05-16" oninput="window._av.filterLogList()"/></div>
      </div>
      <h3 style="color:var(--blue);margin:18px 0 10px">Attendance Records (<span id="att_count">${records.length}</span>)</h3>
      <div id="att_list">
      ${records.length?records.map(r=>{const ru=users.find(x=>x.id===r.userId)||r;return `<div class="list-row att-row" data-search="${esc(((r.name||'')+' '+(r.email||'')+' '+(r.role||'')+' '+(r.at||'')).toLowerCase())}">
        ...
        <div>${r.photo?`<img src="${r.photo}" style="height:54px;width:54px;object-fit:cover;border-radius:8px;cursor:pointer" onclick="window._av.viewAttPhoto('${r.id}')"/>`:''}</div>
      </div>`}).join(''):'<p class="muted">No attendance records yet</p>'}
      </div>
      <h3 style="color:var(--blue);margin:18px 0 10px">🕒 Login / Logout Timings (<span id="log_count">${logs.length}</span>)</h3>
      <div id="log_list">
      ${logs.length?logs.slice(0,300).map(l=>{
        const dur=l.logoutAt?msToHM(new Date(l.logoutAt)-new Date(l.loginAt)):'⏳ still logged in';
        return `<div class="list-row log-row" data-search="${esc(((l.name||'')+' '+(l.email||'')+' '+(l.role||'')+' '+(l.loginAtStr||'')+' '+(l.logoutAtStr||'')).toLowerCase())}"><div><b>${esc(l.name)}</b> <span class="tag approved">${l.role.toUpperCase()}</span>
          <div class="meta">🔑 Login: ${esc(l.loginAtStr)} • 🚪 Logout: ${esc(l.logoutAtStr||'—')}</div></div>
          <div><span class="tag ${l.logoutAt?'success':'pending'}">${dur}</span></div></div>`;
      }).join(''):'<p class="muted">No login history yet</p>'}
      </div>
    `;
  }
}
...
_av.exportAttendance = (kind) => {
  ...
  const records=db.get(KEYS.attendance,[]).filter(r=>r.userId===u.id);
  const logs=db.get(KEYS.sessionsLog,[]).filter(l=>l.userId===u.id);
  if(kind==='excel'){
    ...
    const logRows=[['#','Login','Logout','Duration'],
      ...logs.map((l,i)=>[i+1,l.loginAtStr,l.logoutAtStr||'—',
        l.logoutAt?msToHM(new Date(l.logoutAt)-new Date(l.loginAt)):'still logged in'])];
    const aoa=[...head,['ATTENDANCE MARKS'],...attRows,[],['LOGIN / LOGOUT TIMINGS'],...logRows];
    ...
    y+=6;doc.setFontSize(13);doc.text('Login / Logout Timings',14,y);y+=7;doc.setFontSize(10);
    if(!logs.length){doc.text('No login history.',16,y);y+=8;}
    logs.forEach((l,i)=>{if(y>275){doc.addPage();y=20;}
      const dur=l.logoutAt?msToHM(new Date(l.logoutAt)-new Date(l.loginAt)):'still logged in';
      doc.text(`${i+1}. In: ${l.loginAtStr}  |  Out: ${l.logoutAtStr||'—'}  |  ${dur}`,16,y);y+=6;});
...
_av.filterAttList = () => _filterList('att_search','.att-row','att_count');
